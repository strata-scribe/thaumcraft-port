...
public class FocusEffectAir extends FocusEffect {
    ...
    public boolean execute(HitResult target, @Nullable Trajectory trajectory, float finalPower, int num) {
        if (target instanceof EntityHitResult ehr && ehr.getEntity().level() instanceof ServerLevel serverLevel) {
            ...
            if (trajectory != null) {
                 ehr.getEntity().push(trajectory.direction.x * getSettingValue("power") * 0.2, trajectory.direction.y * getSettingValue("power") * 0.2, trajectory.direction.z * getSettingValue("power") * 0.2);
            }
