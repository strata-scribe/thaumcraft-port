...
public class ResearchAddendum {
	...
	public static final Codec<ResearchAddendum> CODEC = RecordCodecBuilder.create(instance -> instance.group(
		...
		Codec.STRING.listOf().optionalFieldOf("recipes", List.of()).forGetter(a -> {
			if (a.getRecipes() == null) return List.of();
			java.util.List<String> list = new java.util.ArrayList<>();
			for (Identifier id : a.getRecipes()) list.add(id.toString());
			return list;
		}),
		...
		if (!recipes.isEmpty()) {
			...
			if (!ids.isEmpty()) a.setRecipes(ids.toArray(new Identifier[0]));
		}
