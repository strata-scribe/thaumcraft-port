...
public class AspectList implements Serializable {
    ...
    {
        aspects.clear();
        net.minecraft.nbt.ListTag tlist = nbttagcompound.getList("Aspects").orElse(null);
        if (tlist != null) {
			for (int j = 0; j < tlist.size(); j++) {
				CompoundTag rs = tlist.getCompound(j).orElse(null);
				if (rs != null && rs.getString("key").isPresent()) {
					add(	Aspect.getAspect(rs.getString("key").orElse("")),
							rs.getInt("amount").orElse(0));
				}
			}
		}
    }
    ...
    {
        aspects.clear();
        net.minecraft.nbt.ListTag tlist = nbttagcompound.getList(label).orElse(null);
        if (tlist != null) {
			for (int j = 0; j < tlist.size(); j++) {
				CompoundTag rs = tlist.getCompound(j).orElse(null);
				if (rs != null && rs.getString("key").isPresent()) {
