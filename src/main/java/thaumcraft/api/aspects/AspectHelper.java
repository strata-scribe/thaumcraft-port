...
public class AspectHelper {
	...
	public static AspectList getEntityAspects(Entity entity) { 		
		...
		String entityString = net.minecraft.core.registries.BuiltInRegistries.ENTITY_TYPE.getKey(entity.getType()).toString();
	    if (entity instanceof Player) {
			...
			for (ThaumcraftApi.EntityTags et:CommonInternals.scanEntities) {
				...
				if (et.nbts==null || et.nbts.length==0) {
					...
					net.minecraft.world.level.storage.TagValueOutput tvo = net.minecraft.world.level.storage.TagValueOutput.createWithoutContext(net.minecraft.util.ProblemReporter.DISCARDING);
					entity.saveWithoutId(tvo);
					CompoundTag tc = tvo.buildResult();
					for (EntityTagsNBT nbt:et.nbts) {
						if (tc.contains(nbt.name)) {
							if (!ThaumcraftApiHelper.getNBTDataFromId(tc, tc.get(nbt.name).getId(), nbt.name).equals(nbt.value)) continue f1; 
						} else {
							continue f1;
						}
					}
