...
// Removed legacy Forge registries and oredict imports
...
public class ThaumcraftApi {
		...
		for (Object r:getCraftingRecipes().values()) {
			if (r instanceof InfusionRecipe) {
				if (((InfusionRecipe)r).getRecipeOutput() instanceof ItemStack) {
					if (ThaumcraftInvHelper.areItemsEqual((ItemStack) ((InfusionRecipe)r).getRecipeOutput(), res))
	...
	public static CrucibleRecipe getCrucibleRecipe(ItemStack stack) {
		for (Object r:getCraftingRecipes().values()) {
			if (r instanceof CrucibleRecipe) {
				if (ThaumcraftInvHelper.areItemsEqual(((CrucibleRecipe)r).getRecipeOutput(), stack))
	...
	public static boolean exists(ItemStack item) {
		...
		stack.setCount(1);
		AspectList tmp = CommonInternals.objectTags.get(CommonInternals.generateUniqueItemstackId(stack));
		if (tmp==null) {
			try {
				stack.setDamageValue(WILDCARD_VALUE);
				tmp = CommonInternals.objectTags.get(CommonInternals.generateUniqueItemstackId(stack));
				if (item.getDamageValue()==WILDCARD_VALUE && tmp==null) {
					int index=0;
					do {
	 ...
	 * @param item the item passed. Pass WILDCARD_VALUE if all damage values of this item/block should have the same aspects
	 ...
	 * @param item, pass WILDCARD_VALUE to meta if all damage values of this item/block should have the same aspects
	 * @param aspects A ObjectTags object of the associated aspects
	 */
	@Deprecated
	 ...
	 * the crop when fully grown. Sending a metadata of [WILDCARD_VALUE] will mean the metadata won't get 
	 * checked.
	 ...
	 * Sending a metadata of [WILDCARD_VALUE] will mean the metadata won't get checked.
	 ...
	 * As for standard crops, you need to create and ItemStack that tells the golem what block id 
	 * and metadata represents the crop when fully grown. Sending a metadata of [WILDCARD_VALUE] will mean the actualy md won't get 
	 ...
	 * Sending a metadata of [WILDCARD_VALUE] will mean the metadata won't get checked.
	 * Example for vanilla wheat: 
	 * FMLInterModComms.sendMessage("Thaumcraft", "lampBlacklist", new ItemStack(Block.crops,1,WILDCARD_VALUE));
	 */
